...
if (isset($_GET['index']) and $_GET['index'] != '') {
    ...
    $id_cate = htmlspecialchars(trim($_GET['index']));

    $verifie_cat0 = $lock->prepare('SELECT * FROM commerce_categorie WHERE Id = ? ');
    $verifie_cat0->execute(array($id_cate));
    $verifie_cat = $verifie_cat0->fetch();

    if (!isset($verifie_cat['Id'] ) or $verifie_cat['Id'] == '' ) {
...
<?php
    ...
    while ($categorie = $categorie0->fetch()) {
        ...
        if ( $categorie['Id'] == $verifie_cat['Grande'] ) {
            $active_gr = "active";
        }else { $active_gr = ""; }
        ...
        while ($cat = $cat0->fetch()) {  
            ...
            if ( $cat['Id'] == $verifie_cat['Id'] ) {
                $active_pt = "active";
            }else { $active_pt = ""; }  

            echo '
                                    <li style="text-align: left;" class="'.$active_pt.'" ><a href="categorie.php?index='.$cat['Id'].'&cat='.$cat['Grande'].'&token='.sha1($index_cat.time().$index_cat).'"><b>'.$cat['Nom'].'</b></a></li>
            '; 
            $index_cat++;
        }
...
?>                                                  
        ...
        <main class="main" style="background-image: url(image/fond.jpg);">
        	<div class="page-header text-center" style="background-image: url('assets/images/page-header-bg.jpg')">
        		<div class="container">
        			<h1 class="page-title"> <?php echo $verifie_cat['Nom'];  ?> </h1>
        		</div><!-- End .container -->
        	</div><!-- End .page-header -->
...
echo '<div id="divIdCategorie" style="display:none;">'.$verifie_cat['Id'].'</div>';

$select0 = $lock->prepare('SELECT art.*, cat.Nom AS Nom_cat FROM commerce_article AS art INNER JOIN commerce_categorie AS cat WHERE art.Categorie = cat.Id AND art.Categorie = ? ORDER BY RAND() LIMIT 12 ');

$select0->execute(array($verifie_cat['Id']));
...
<script type="text/javascript">
    ...
    $('#voir_plus').click(function(e){
        ...
        var id_cat = $('#divIdCategorie').text();

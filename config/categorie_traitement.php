...
if (isset($_POST['voir_plus']) AND $_POST['voir_plus'] == 'oui' ) {
	...
	$select0 = $lock->prepare("SELECT art.*, cat.Nom AS Nom_cat FROM commerce_article AS art INNER JOIN commerce_categorie AS cat WHERE art.Categorie = cat.Id AND art.Categorie = ? AND art.Id NOT IN $tableau ORDER BY RAND() LIMIT 12 ");
	$select0->execute(array($_POST['id_cat']));
